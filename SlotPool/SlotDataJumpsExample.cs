...
public class SlotDataJumpsExample : UdonSharpBehaviour
{
    #region BaseClass
    ...
    public uint jumps;
    [UdonSynced]
    uint jumpsSerializationCount;
    uint jumpsDeserializationCount;

    bool initialized;

    public DebugLogger debug;
    public UintScoreboardExample scoreboard;
    ...
    {
        jumpsDeserializationCount = jumpsSerializationCount;
    }

    // Regular variable deserialization
    void _u_OnDeserialization()
    {
        if (jumpsDeserializationCount != jumpsSerializationCount)
        {
            jumpsDeserializationCount = jumpsSerializationCount;
            // Activate any external events that rely on jump changing.
            debug._u_Log(Networking.GetOwner(gameObject).displayName + " jumps: " + jumps);
