...
namespace Udon_MIDI_Web_Helper
{
    class LogParser
    {
        ...
        void ProcessLogLine(string line)
        {
            ...
            // 2021.01.01 00:00:00 Log        -  [Udon-MIDI-Web-Helper] GET 0 https://www.vrchat.com
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] RDY
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] ACK
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] WSO 1 wss://whatever.com
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] WSM 1 txt MessageText
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] WSM 1 bin binaryblob
            // 2021.03.16 20:00:01 Log        -  [Udon-MIDI-Web-Helper] WSC 1
            if (line.Length > 56 && line.Substring(34, 22) == "[Udon-MIDI-Web-Helper]")
            {
                string[] args = line.Substring(57).Split(' ');
                switch (args[0])
                {
                    case "GET": // new http get request with conntionID and uri argument
                        {
                            ...
                            webManager.GetWebRequest(connectionID, uriDecoded);
                            break;
                        }
                    case "RDY":
                    ...
                    case "ACK":
                        ...
                        break;
                    case "WSO": // new websocket connection with conntionID and url arguments
                        {
                            Console.WriteLine(line);
                            int connectionID = Int32.Parse(args[1]);
                            string uriDecoded = Encoding.Unicode.GetString(Convert.FromBase64String(args[2]));
                            webManager.OpenWebSocketConnection(connectionID, uriDecoded);
                            break;
                        }
                    ...
                    case "WSM": // Send websocket message with connectionID, text/bin flag, and data arguments
                        {
                            Console.WriteLine(line);
                            int connectionID = Int32.Parse(args[1]);
                            bool textMessage = args[2] == "txt";
                            byte[] data = Convert.FromBase64String(args[3]);
                            webManager.SendWebSocketMessage(connectionID, data, textMessage);
                            break;
                        }
                }
            }
